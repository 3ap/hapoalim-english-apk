...
def generate_xml(strings, translates, language):
    for obj in strings.getElementsByTagName('string'):
        if obj.firstChild != None:
            key = obj.getAttribute("name")
            if key in translates and language in translates[key]:
                obj.firstChild.nodeValue = translates[key][language].replace("'", "\\'")
    print(strings.toxml())
...
if __name__ == "__main__":
    strings = minidom.parse('strings.xml')

    translates = {}
    for obj in strings.getElementsByTagName('string'):
        key = obj.getAttribute("name")
        if obj.firstChild != None:
            translates[key] = { "hebrew": obj.firstChild.nodeValue }

    with open('strings.xml.csv') as f:
